...
import io.github.konradmalik.util.{StringUtil, Using}
...
trait CassandraEnv extends CassandraConfig {
  ...
  final def getKeyspaces: Seq[String] = {
    withCassandraCluster { cluster =>
      ...
      val set = metadata.getKeyspaces.asScala.map(_.getName).toSet.diff(KEYSPACES_TO_OMIT)
      set.toSeq.sorted
    }
  }
   ...
   * @return [[ResultSet]]
   */
  final def initializeRole(name: String)(implicit session: Session): ResultSet = {
    assert(StringUtil.isAlphanumeric(name), s"only alphanumeric characters allowed for roles! was $name")
   ...
   * @return [[ResultSet]]
   */
  final def assignAdminRightsTo(role: String, keyspace: String)(implicit session: Session): ResultSet = {
    assert(StringUtil.isAlphanumeric(keyspace), s"only alphanumeric characters allowed for keyspaces! was $keyspace")
   ...
   * @return Query
   */
  private final def assignAdminRightsToQuery(role: String, keyspace: String): String =
   ...
   * @return [[ResultSet]]
   */
  final def initializeKeyspace(keyspace: String, replication: Int = CASSANDRA_REPLICATION)(implicit session: Session): ResultSet = {
   ...
   * @return Query
   */
  private final def initializeKeyspaceQuery(keyspace: String, replication: Int = CASSANDRA_REPLICATION): String =
