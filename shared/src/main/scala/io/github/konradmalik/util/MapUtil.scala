...
object MapUtil {
  ...
  def mergeToColletion[K, V](m1: Map[K, V], m2: Map[K, V]): Map[K, List[V]] =
    (m1.keySet ++ m2.keys) map { i => i -> (m1.get(i).toList ::: m2.get(i).toList) } toMap
   ...
   * @return Merged map
   */
  def mergeWithCollections[K, V](m1: Map[K, Iterable[V]], m2: Map[K, Iterable[V]]): Map[K, Iterable[V]] =
    (m1.keySet ++ m2.keys) map { k => k -> (m1.getOrElse(k, Iterable.empty) ++ m2.getOrElse(k, Iterable.empty)) } toMap
