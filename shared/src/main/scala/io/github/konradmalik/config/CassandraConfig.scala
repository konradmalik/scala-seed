...
trait CassandraConfig {
   ...
   */
  lazy final val QUERY_TIMEOUT = Duration.fromNanos(config.getDuration("queryTimeout").toNanos)
}
...
private object CassandraConfig {
  private lazy final val config = TypesafeConfig.coreConfig.getConfig("cassandra")
}
