...
import io.github.konradmalik.config.SparkConfig
...
trait SparkEnv {
  ...
  /**
    ...
    */
  final def withSparkSession[T](f: SparkSession => T): T =
    Using.withResources(createSparkSession)(f)
}
...
private object SparkEnv extends SparkConfig with CassandraConfig {
  ...
  /**
    ...
    */
  final def createSparkSession: SparkSession =
    createSparkSession(SPARK_APP_NAME)
